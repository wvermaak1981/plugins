...
final class WC_CSV_Product_Feed_Importer {
    ...
    public function render_page() {
        ...
        <div class="wrap">
            ...
            <?php elseif ('error' === $notice) : ?>
                <div class="notice notice-error is-dismissible"><p><?php echo esc_html('No readable CSV data was found. Check the file, URL and CSV header.'); ?></p></div>
    ...
    public function handle_import() {
        ...
        if (!empty($file['error']) && UPLOAD_ERR_NO_FILE !== (int) $file['error']) {
            wp_die('CSV upload failed. Error code: ' . absint($file['error']));
        }
    ...
    private function import_rows($rows) {
        ...
        foreach ($rows as $row) {
            try {
                if ($this->import_row($row)) { $imported++; } else { $failed++; }
            } catch (Throwable $e) {
                $failed++;
                error_log('CSV Feed Importer: ' . $e->getMessage());
            }
        }
    ...
    private function import_row($row) {
        ...
        $categories = preg_split('/\s*[|,>\/]+\s*/', (string) ($row['category'] ?? ''), -1, PREG_SPLIT_NO_EMPTY);
        $term_ids = [];
        ...
        if (!empty($row['image'])) {
            $image_id = $this->import_image($row['image'], $id);
            if ($image_id) { $product->set_image_id($image_id); $product->save(); }
        }
        return true;
    }

    private function import_image($url, $product_id) {
    ...
    private function decimal($value) {
        ...
        return ('' === $value || '-' === $value || '.' === $value) ? '' : number_format((float) $value, 2, '.', '');
    }
}

add_action('plugins_loaded', static function () {
    if (class_exists('WooCommerce')) { WC_CSV_Product_Feed_Importer::instance(); }
});
