...
final class WPFI_Importer {
    ...
    public function import(array $feed) {
        ...
        $response = wp_remote_get($feed['url'], ['timeout'=>90, 'redirection'=>3, 'user-agent'=>'WPFI/'.WPFI_VERSION]);
        ...
        foreach ($nodes as $node) {
            ...
            $identifier_field = $feed['identifier_field'] ?? 'sku';
            $identifier = trim((string)($values[$identifier_field] ?? ($values['sku'] ?? '')));
            if (!$identifier) { $this->logger->write($feed, 'warning', 'Skipped a product without an identifier.'); continue; }
            $existing = $identifier_field === 'sku' ? wc_get_product_id_by_sku($identifier) : $this->find_meta_product($identifier_field, $identifier);
            ...
            $this->apply_category($product_id, $values['category'] ?? '');
            $this->apply_image($product_id, $values['image'] ?? '');
            if ($is_variable) {
                $product = wc_get_product($product_id);
                $product->set_attributes($this->variation_attributes($node, $feed['attributes'] ?? []));
                $product->save();
    ...
    private function variation_attributes($node,$attributes) { $out=[]; foreach ((array)$attributes as $name=>$selector) { $value=$this->value($node,$selector); if ($value==='') continue; $taxonomy=$this->taxonomy($name); $out[$taxonomy]=['name'=>$taxonomy,'value'=>$value,'is_visible'=>1,'is_variation'=>1,'is_taxonomy'=>1]; } return $out; }
    private function save_variation($parent_id,$node,$feed) {
        ...
        if (!empty($values['image'])) $this->apply_image($variation->get_id(), $values['image']);
    ...
    private function apply_category($product_id,$value) { if (!$value || !$product_id) return; wp_set_object_terms($product_id, array_filter(array_map('trim', preg_split('/[,|]/', $value))), 'product_cat', false); }
    private function apply_image($product_id,$url) {
        if (!$product_id || !$url || has_post_thumbnail($product_id)) return;
        ...
        $attachment=media_sideload_image(esc_url_raw($url), $product_id, '', 'id');
        if (!is_wp_error($attachment) && $attachment) set_post_thumbnail($product_id, $attachment);
    }
}
